    ...
    private:
        ...
        std::unique_ptr<Mesh> processMesh(aiMesh *mesh, const aiScene *scene, bool usePBR)
        {
            ...
            if (!usePBR)
            {
                ...
                return std::make_unique<Mesh>(vertices, indices, textures, usePBR);
            }
            else
            {
                PBRMaterial pbrMat;
                ...
                {
                    std::vector<std::shared_ptr<Renderer::Texture>> aoMaps = loadMaterialTextures(material, aiTextureType_AMBIENT_OCCLUSION, "material.aoMap");
                ...
                // return a mesh object created from the extracted mesh data
                return std::make_unique<Mesh>(vertices, indices, textures, usePBR, pbrMat);
            }
        }
